...
import { url_api } from '../../helpers/reporte_data'
import { url_reportes_periodo } from '../../helpers/reporte_data'
...
function ListaReportes() {
  const {data, consulta, setConsulta, setUrl, loading} = useReporte()
  ...
  const handlerClick = (event) => {
    ...
    setUrl(`${url_reportes_periodo}/q=${fechas[1]},${fechas[0]}`)
          ...
          <table className='block'>
              <thead className='text-sm font-medium text-slate-200'>
                ...
                <tr className='text-slate-700'>
                  <th> {data.length === 1 ? null : <Filter query='contratista_nombre_comercial' datos={ data } consultaQuery={consultaQuery} updateUrl={updateUrl} url={url_api} consulta={consulta}/>} </th>
                  <th> {data.length === 1 ? null : <Filter query='Proyecto' datos={ data } consultaQuery={consultaQuery} updateUrl={updateUrl} url={url_api} consulta={consulta}/>} </th>
                  <th> {data.length === 1 ? null : <Filter query='documento' datos={ data } consultaQuery={consultaQuery} updateUrl={updateUrl} url={url_api} consulta={consulta}/>} </th>
                  <th> {data.length === 1 ? null : <Filter query='estatus' datos={ data } consultaQuery={consultaQuery} updateUrl={updateUrl} url={url_api} consulta={consulta}/>} </th>
                  <th> {data.length === 1 ? null : <Filter query='trabajado' datos={ data } consultaQuery={consultaQuery} updateUrl={updateUrl} url={url_api} consulta={consulta}/>} </th>
                </tr>
              </thead>
          </table>
        </div>
...
export default React.memo(ListaReportes)